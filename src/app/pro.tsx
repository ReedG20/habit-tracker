...
/**
 * The paywall. Prices come from the store through RevenueCat, never from
 * code, so a price change in App Store Connect needs no release. Errors are
 * rendered inline: a UIKit sheet sits above the toast host.
 */
    ...
    <ScrollView contentContainerStyle={styles.sheet} alwaysBounceVertical={false}>
      ...
      {isPro && !previewing ? (
        <>
          ...
          <ActionButton key="done" label="Done" variant="primary" fill onPress={dismiss} />
          {__DEV__ && (
            <ActionButton
              key="preview"
